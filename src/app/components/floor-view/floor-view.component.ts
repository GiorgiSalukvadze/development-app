...
export class FloorViewComponent implements OnInit {
  ...
  getUnitColor(unit: Unit): string {
    if (this.showLeads && unit.salesLeads && unit.salesLeads.length > 0) {
  ...
  getUnitHoverColor(unit: Unit): string {
    if (this.showLeads && unit.salesLeads && unit.salesLeads.length > 0) return 'rgba(234, 179, 8, 0.8)';
  ...
  getLeadCount(unit: Unit): number {
    return unit.salesLeads ? unit.salesLeads.length : 0;
  }

  getInterestCount(unit: Unit, interest: 'high' | 'medium' | 'low'): number {
    if (!unit.salesLeads) return 0;
    return unit.salesLeads.filter(l => l.interest === interest).length;
  }
}